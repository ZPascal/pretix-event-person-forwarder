...
class Api:
    ...
    def call_the_api(
        ...
        api_url: str = f"{self.pretix_api_model.host}{api_call}"

        headers: dict = dict()
        if self.pretix_api_model.headers is not None:
            headers: dict = self.pretix_api_model.headers
    ...
    def create_the_http_api_client(
        ...
        if http2:
            async_transport: httpx.AsyncHTTPTransport = httpx.AsyncHTTPTransport(
                retries=self.pretix_api_model.retries, http2=http2
            )
            return httpx.AsyncClient(
                http2=True,
                limits=limits,
                timeout=self.pretix_api_model.timeout,
                headers=headers,
